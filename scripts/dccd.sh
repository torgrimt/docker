...
update_compose_files() {
	...
	find -L . \( -name 'docker-compose.yml' -o -name 'docker-compose.yaml' -o -name 'compose.yaml' -o -name 'compose.yml' \) | grep -v apps | sort | while IFS= read -r file; do
		...
		if [ $GRACEFUL -eq 1 ]; then
			      ...
			      c=$(echo $file | awk -F "\/" '{print $3}')
                              /usr/bin/curl -H "Title: dccd $(hostname -s)" -d "Oppdaterte $c" https://ntfy.teigstad.no/test >/dev/null 2>&1
                         else
                              log_message "GRACEFUL: Skipping Redeploying compose file for $file (no change)"
                         fi
		else
